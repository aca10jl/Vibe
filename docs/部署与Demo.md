...
| 模式 | 命令 | AI 能力 | 需要 |
|---|---|---|---|
| **纯静态**（最快） | `./start.sh static` | 本地推理引擎 | 仅浏览器 / python3 |
| **全功能** | `./start.sh` | 本地推理 + 可选 Claude 在线 | Node ≥ 20 |

> Demo 默认即可运行：**不配置任何 API Key**，右侧助手就能伴读/分析/建议并操作页面。
> 配置 Key 后，把右上角开关切到 **Claude**，即走真实大模型。
...
健康检查：`curl localhost:8787/api/health` → `{"ok":true,"model":"claude-opus-4-8","claude":true}`
...
6. **切换 Claude**（如已配 Key）：右上开关切到 Claude，重复 2–4，对比真实大模型的措辞与工具决策。
